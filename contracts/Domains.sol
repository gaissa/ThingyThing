...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Thingy Thing Name Service", "TTNS") {
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Thingy Thing name service", "image": "data:image/svg+xml;base64,',
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    require(len > 0);
    if (len == 3) {
      return 0.01 * 10**17; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
    } else if (len == 4) {
      return 0.005 * 10**17; // To charge smaller amounts, reduce the decimals. This is 0.3
    } else {
      return 0.0025 * 10**17;
    }
  }
